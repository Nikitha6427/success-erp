...
class _PoDetailScreenState extends ConsumerState<PoDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final items =
          await ref.read(poItemRepositoryProvider).loadByPoId(widget.id);
  ...
  bool get _hasInvoiceable => _items.any((i) =>
  ...
  Widget _linkedDeliveryNotes() {
    final linked =
        ref.watch(dnListProvider).where((d) => d.poId == widget.id).toList();
    if (linked.isEmpty) {
